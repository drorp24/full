...
const FormContainer = ({ structure, show }) => {
  ...
  // ! let render await useEffect - and stop server rendering
  // The following line ensures that render awaits form's state population.
  // Effectively, it also prevets ssr from rendering any further.
  // The very same reason this line exists, which is to ensure render awaits, will also cause the server to never reach beyond that point.
  // That's since server rendering, unlike client rendering, isn't get called again when any component dependency (here: 'populated') is changed.
  //
  // Of course I could make the server await form state population to complete then go on rendering, but that would defeat the purpose which is performance boost.
  // I think the right way to use the server's ability to generate html w/o paying a performance penalty on lengthy effects (which would actualy worsen performance)
  // is to make some static entry page with little or no effects (at least not asynchronic ones that requires awaiting)
  // and have the server render and serve it to the client with the initial html, before the JS chunk has been loaded - which was the whole point behind ssr.
  //
  // To explicitly prevent any component from server rendering, one can use Abramov's 'showChild' trick:
  // set this state variable to true in a useEffect, and don't render the given component unless it's true.
  // The fact that useEffect runs only in the client will prevent that component from being generated by the server.
  //
  console.log('before the if.populated: ', populated)
